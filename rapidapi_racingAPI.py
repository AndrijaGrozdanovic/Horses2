...
def get_turf_handicap_racecards(day="tomorrow"):
    ...
    for race in data.get("racecards", []):
        ...
        handicap = (
            "Hcap"
            if "handicap" in race_name.lower()
            else "NonHcap"
        )
        ...
        race_info = {
            ...
            "yards": int(
                float(race.get("distance_f")) * 220
                if race.get("distance_f")
                else None
            ),
            ...
            "ORLimitNumerical": (
                str(race.get("rating_band")).split('-')[-1]
                if race.get("rating_band")
                else ""
            ),
...
#     obj.createConnection("AUTOCOMMIT")
#     con = obj.connection
#     df = get_turf_handicap_racecards('today')
#     DbConnection.importToTable('Race_Card_turf_handicap', con, df)
